...
if args.graph == 'default':
    ...
    W = weight_matrix(pjoin('./dataset', f'fire_conus_csv.csv'))
else:
...
data_file = f'fire_conus_csv.csv'
n_train, n_val, n_test = 0.8 * n, 0.16 * n, 0.04 * n
#PeMS = data_gen(pjoin('./dataset', data_file), (n_train, n_val, n_test), n, n_his + n_pred)
#print(f'>> Loading dataset with Mean: {PeMS.mean:.2f}, STD: {PeMS.std:.2f}')

Wildfire = data_gen(pjoin('./dataset', data_file), (n_train, n_val, n_test), n, n_his + n_pred)

if __name__ == '__main__':
    #model_train(PeMS, blocks, args)
    model_train(Wildfire, blocks, args)
    #model_test(PeMS, PeMS.get_len('test'), n_his, n_pred, args.inf_mode)
    model_test(Wildfire, Wildfire.get_len('test'), n_his, n_pred, args.inf_mode)
